    ...
    return (
      <div className="RatioChart" id="chart">
        ...
        <div className="RatioChart-container RatioChart-card">
          {data.length === 0
            ...
            : <XYPlot
              ...
              >
                <HorizontalGridLines />
                <XAxis title={fieldLabels[xField]} tickTotal={5} />
                <YAxis title={fieldLabels[yField]} tickTotal={8} />
          ...
          <div className="RatioChart-settings__items">
            {Object.keys(mappedData).map((key, index) => (
              <div key={"checkbox-" + key}>
                ...
                />
                <label
                  htmlFor={"type-checkbox-" + key}
                  onMouseEnter={this._highlightType}
                  onMouseLeave={this._unhighlightType}
                  data-type={key}
                  style={{ color: getColorByIndex(index) }}
                >
                  {categoryLabels[key]} ({mappedData[key].length})
                </label>
              </div>
            ))}
          </div>
        ...
        <div className="RatioChart-add">
          Want to add the project to chart?{" "}
          <a
            target="_blank"
            href="https://github.com/StarRatio/star-ratio/edit/master/docs/data.json"
          >
